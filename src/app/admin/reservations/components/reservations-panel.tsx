...
import { columns, type ReservationWithDetails } from "./columns";
  ...
  const groupedReservations = React.useMemo(() => {
    return bookings.map((booking) => {
      ...
      if (booking.subRows && booking.subRows.length > 0) {
        detailedBooking.subRows = (
          booking.subRows as unknown as ReservationWithDetails[]
        ).map((sub) => {
          ...
          if (isRevenueReservation(sub.status)) {
            subWithDetails.displayAmount = getReservationDisplayAmount(
              subWithDetails,
              property,
            );
            const { paidAmount, remainingBalance } =
              getReservationPaidAndBalance(subWithDetails, property);
        ...
        if (detailedBooking.subRows && detailedBooking.subRows.length > 0) {
          detailedBooking.displayAmount = detailedBooking.subRows.reduce(
            (sum, sub) => sum + (sub.displayAmount || 0),
            0,
          );
          detailedBooking.paidAmount = detailedBooking.subRows.reduce(
            (sum, sub) => sum + (sub.paidAmount ?? 0),
            0,
          );
          detailedBooking.remainingBalance = detailedBooking.subRows.reduce(
            (sum, sub) => sum + (sub.remainingBalance ?? 0),
            0,
          );
        } else {
          detailedBooking.displayAmount = isRevenueReservation(
      ...
      } else if (isRevenueReservation(detailedBooking.status)) {
        detailedBooking.displayAmount = getReservationDisplayAmount(
          detailedBooking,
          property,
        );
        const { paidAmount, remainingBalance } = getReservationPaidAndBalance(
          detailedBooking,
          property,
        );
