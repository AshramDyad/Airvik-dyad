...
export default function CreateReservationForm() {
  ...
  const {
    ...
    isLoading: isLoadingGuestSearch,
  } = useGuestsPage({
    limit: 50,
    offset: 0,
  ...
  }, [conflictingRoomIds, isLoadingRoomConflicts, rooms, watchedDateRange]);
  ...
  const {
    ...
    isLoading: isLoadingSelectedGuest,
  } = useGuestProfile(selectedGuestId);
  ...
  const selectedGuest = React.useMemo(
    () =>
      guestSearchResults.find((guest) => guest.id === selectedGuestId) ??
      selectedGuestProfile,
    [guestSearchResults, selectedGuestId, selectedGuestProfile],
  );

  const guestOptions = React.useMemo(() => {
    if (
      !selectedGuest ||
      guestSearchResults.some((guest) => guest.id === selectedGuest.id)
    ) {
      return guestSearchResults;
    }

    return [selectedGuest, ...guestSearchResults];
  }, [guestSearchResults, selectedGuest]);
  ...
  const guestButtonLabel =
    selectedGuestLabel ||
    (selectedGuestId && isLoadingSelectedGuest
      ? "Loading guest..."
      : "Select guest");
