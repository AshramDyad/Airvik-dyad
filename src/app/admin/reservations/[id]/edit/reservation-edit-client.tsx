...
const ReservationEditForm = dynamic(
  () =>
    import("@/app/admin/reservations/components/reservation-edit-form").then(
      (module) => module.ReservationEditForm,
    ),
  { loading: () => <ReservationEditFormSkeleton /> },
);
...
export function ReservationEditClient() {
  ...
  const bookingRowRoomNumber =
    "roomNumber" in reservation && typeof reservation.roomNumber === "string"
      ? reservation.roomNumber
      : undefined;
  const roomNumber =
    activeBookingRooms.find((room) => room.id === reservation.roomId)?.roomNumber ??
    bookingRowRoomNumber ??
    "N/A";
...
function ReservationEditFormSkeleton() {
  return (
    <div className="grid gap-6 lg:grid-cols-5">
      <Skeleton className="h-[520px] w-full rounded-2xl lg:col-span-3" />
      <Skeleton className="h-[520px] w-full rounded-2xl lg:col-span-2" />
