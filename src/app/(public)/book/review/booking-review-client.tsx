...
import { useBookingReviewData } from "@/hooks/use-booking-review-data";
...
import { getCountryPincodeConfig } from "@/lib/country-config";
import { validatePincodeByCountry } from "@/lib/validators/country-validation";
import type { RoomType } from "@/data/types";

const CountryCombobox = dynamic(
  () =>
    import("@/components/ui/country-combobox").then(
      (module) => module.CountryCombobox,
    ),
  {
    loading: () => <div className="h-12 rounded-xl border border-border/50 bg-card/80" />,
  },
);
...
function BookingReviewContent() {
  ...
  const { isLoading: isPropertyLoading, property } = useDataContext();
  ...
  const {
    ...
    checkIn: bookingDetails.from,
    checkOut: bookingDetails.to,
  });
  const reviewRoomTypes = reviewData?.roomTypes ?? [];
  const reviewSeasonalPrices = reviewData?.seasonalPrices ?? [];
  const reviewPropertyClosures = reviewData?.propertyClosures ?? [];

  const selectedRoomTypes = React.useMemo(() => {
    if (!selectedRoomTypeIds.length) return [];
    return selectedRoomTypeIds
      .map((id) => reviewRoomTypes.find((rt) => rt.id === id))
      .filter(Boolean) as RoomType[];
  }, [selectedRoomTypeIds, reviewRoomTypes]);
  ...
  const groupedRoomTypes: SelectedRoomTypeSummary[] = React.useMemo(() => {
    ...
    counts.forEach((quantity, roomTypeId) => {
      const roomType = reviewRoomTypes.find((rt) => rt.id === roomTypeId);
      if (!roomType) return;
  ...
  const ratePlan = reviewData?.ratePlan ?? undefined;
  ...
  const pricing = React.useMemo(() => {
    ...
    if (selectedRoomTypes.length === 1) {
      return calculateRoomPricing({
        ...
        nights,
        taxConfig,
        seasonalPrices: reviewSeasonalPrices,
        checkInDate,
      });
    } else {
  ...
  }, [selectedRoomTypes, ratePlan, nights, taxConfig, reviewSeasonalPrices, bookingDetails.from]);
  ...
  const activeHeroRoomType = React.useMemo(() => {
    if (activeRoomTypeId) {
      const found = reviewRoomTypes.find((rt) => rt.id === activeRoomTypeId);
      if (found) {
        return found;
      }
    }
    return primaryRoomType;
  }, [activeRoomTypeId, primaryRoomType, reviewRoomTypes]);
  ...
  if (isPropertyLoading || isReviewDataLoading) {
  ...
  async function onSubmit(values: z.infer<typeof paymentSchema>) {
    ...
    try {
      ...
      const blockingClosure = reviewPropertyClosures.find((closure) => {
      ...
      const response = await fetch("/api/bookings/public", {
        ...
        body: JSON.stringify({
          ...
          specialRequests: additionalRequest || undefined,
          guest: {
      ...
      const payload = await response
        .json()
        .catch(() => null) as {
          data?: { confirmationReservationId?: string };
          message?: string;
          code?: string;
        } | null;
      ...
      if (!response.ok) {
        const error = new Error(
          payload?.message || DEFAULT_BOOKING_ERROR_MESSAGE,
        ) as Error & { code?: string };
        error.code = payload?.code;
        throw error;
      }

      const confirmationReservationId =
        payload?.data?.confirmationReservationId;

      if (!confirmationReservationId) {
        throw new Error("Could not create reservation.");
      }

      router.push(`/book/confirmation/${confirmationReservationId}`);
    } catch (error: unknown) {
      ...
      const isConflict =
        ...
        error !== null &&
        "code" in error &&
        ["23P01", "ROOM_UNAVAILABLE"].includes(
