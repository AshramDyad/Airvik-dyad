...
import type { Amenity, PropertyClosure, RoomType } from "@/data/types";
...
const RoomTypeCard = dynamic(
  ...
  {
    loading: () => <div className="h-64 rounded-2xl bg-muted/30" />,
  },
);

const BookingSummary = dynamic(
  () =>
    import("@/components/public/booking-summary").then(
      (module) => module.BookingSummary,
    ),
  { ssr: false },
);

const EMPTY_ROOM_TYPES: RoomType[] = [];
const EMPTY_AMENITIES: Amenity[] = [];
...
export function BookingClient() {
  const {
    bookingSearchData,
    isLoading: isBookingSearchDataLoading,
  } = useBookingSearchData();
  const bookingRoomTypes = bookingSearchData?.roomTypes ?? EMPTY_ROOM_TYPES;
  const bookingClosures =
    bookingSearchData?.propertyClosures ?? EMPTY_PROPERTY_CLOSURES;
  const visibleRoomTypes = React.useMemo(
    () => bookingRoomTypes.filter((roomType) => roomType.isVisible !== false),
    [bookingRoomTypes],
  );
  const {
    search,
    ...
    propertyClosures: bookingClosures,
  });
  ...
  const showLoading = isBookingSearchDataLoading || isSearching;
                      ...
                      return (
                        <div key={roomType.id} className="space-y-2">
                          <RoomTypeCard
                            roomType={roomType}
                            price={displayPrice}
                            amenities={bookingSearchData?.amenities ?? EMPTY_AMENITIES}
      ...
      {hasSearched &&
        ...
        !hasInsufficientTotalCapacity && (
          <BookingSummary
            ...
            searchValues={searchValues}
            ratePlan={bookingSearchData?.ratePlan ?? null}
