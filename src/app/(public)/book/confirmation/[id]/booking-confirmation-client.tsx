...
export function BookingConfirmationClient() {
  ...
  const [confirmationData, setConfirmationData] =
    React.useState<BookingConfirmationPayload | null>(null);
  const [isLoadingReservation, setIsLoadingReservation] =
    React.useState(false);
  const [hasAttemptedFetch, setHasAttemptedFetch] = React.useState(false);

  const reservation = confirmationData?.reservation ?? null;
  const guest = confirmationData?.guest ?? null;
  const bookingReservations =
    confirmationData?.bookingReservations ?? (reservation ? [reservation] : []);
  const rooms = confirmationData?.rooms ?? [];
  const roomTypes = confirmationData?.roomTypes ?? [];
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/bookings/confirmation/${encodeURIComponent(reservationIdFromParams)}`, {
      cache: "no-store",
    })
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(payload?.message || "Unable to load reservation.");
        }
